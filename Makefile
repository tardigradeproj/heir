...
KIND_CLUSTER ?= samaritano-test-e2e
...
.PHONY: run-distro
run-distro:
	go run cmd/distro.go build control-plane-image --base-image ghcr.io/tardigrade/samaritano-base:latest --type=file $$(pwd)/artifacts/kubernetes-server-linux-arm64.tar.gz
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name samaritano-builder
	$(CONTAINER_TOOL) buildx use samaritano-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm samaritano-builder
	rm Dockerfile.cross
