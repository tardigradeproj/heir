...
type WorkerJoinTokenSpec struct {
	// TTL is how long a minted token remains valid on this cluster, starting
...
type WorkerJoinTokenStatus struct {
	...
	//   Degraded — the controller could not resolve cluster info or mint a token.
	//
	...
	// ExpiresAt is when the currently issued token expires.
	// +optional
	ExpiresAt *metav1.Time `json:"expiresAt,omitempty"`

	// SecretRef names the Secret, in the kube-system namespace, holding the
